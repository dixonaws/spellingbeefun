...
app = Chalice(app_name='spellingbeefun-api')

@app.route('/{letters}', methods=['GET'], cors=True)
def index(letters):
    if(letters == ""):
        raise BadRequestError("You must specify letters to use in the solution, beginning with the required letter, for example curl https://apiurl.com/abcdefgh")

    str_bucket_name="spellingbeefun"
    str_wordlist_filename="clean_wordlist.txt"

    dict_solution={}
    dict_solution['letters']=letters
    dict_solution['required_letter']=letters[0]

    print("Downloading word list from S3...")
    s3_client=boto3.client("s3")
    s3_client.download_file(str_bucket_name, str_wordlist_filename, "/tmp/" + str_wordlist_filename)

    file_words = open("/tmp/" + str_wordlist_filename, "r")

    lst_words = file_words.read().splitlines()
    file_words.close()

    print("Loaded " + str(len(lst_words)) + " possible solutions from " + str_wordlist_filename)

    lst_letters=[]
    for letter in letters:
        lst_letters.append(letter)

    lst_alphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't',
                    'u',
                    'v', 'w', 'x', 'y', 'z']
    ...
    dict_solution['solution_set']=lst_solutions
